...
@app.get("/api/packages/{pid}/feedback")
...
@app.get("/api/packages/{pid}/debriefs")
...
@app.post("/api/packages/{pid}/debriefs")
...
async def finance_home(user=Depends(authenticate_token)):
    ...
    utang vendor + antrian approval yang menunggu Finance action (refund cair,
    komisi cair, expense pay)."""
    require_role(user, "admin", "finance", "management")
    ...
    if refund_pending:
        attention.append({
            "key": "refund_pending",
            "severity": "high",
            "label": f"{refund_pending} refund menunggu dicairkan",
            "goto": "finance",
        })
    if komisi_pending:
        attention.append({
            "key": "komisi_pending",
            "severity": "high",
            "label": f"{komisi_pending} klaim komisi agen menunggu dicairkan",
            "goto": "finance",
        })
    if expense_pending:
        attention.append({
            "key": "expense_pending",
            "severity": "medium",
            "label": f"{expense_pending} expense report menunggu proses",
            "goto": "expense",
        })

    # Vendor due 7-14 hari
    ...
    if vendor_due_soon:
        ...
        if overdue:
            attention.append({
                "key": "vendor_overdue",
                "severity": "critical",
                "label": f"{len(overdue)} pembayaran vendor sudah lewat jatuh tempo",
                "goto": "finance",
            })
        else:
            attention.append({
                "key": "vendor_due_soon",
                "severity": "medium",
                "label": f"{len(vendor_due_soon)} vendor payment jatuh tempo ≤14 hari",
                "goto": "finance",
            })

    # Piutang jamaah top 10 by sisa
    ...
    if piutang_urgent:
        attention.append({
            "key": "piutang_urgent",
            "severity": "critical",
            "label": f"{len(piutang_urgent)} jamaah belum lunas, berangkat ≤30 hari",
            "goto": "finance",
        })

    # Aged receivable buckets
...
async def mgmt_monthly_pdf(month: str = None, user=Depends(authenticate_token)):
    ...
    for s in sales_users:
        ...
        sales_perf.append({
            "name": s["name"],
            "actual_closing": c_actual,
            "actual_omzet": o_actual,
    ...
    top_agents = db.query_all(
        ...
        "GROUP BY a.id ORDER BY omzet DESC LIMIT 10",
        (ym,),
    )
    ...
    payload = {
        ...
        "kpi": {
            "revenue": m_now["omzet"],
            "revenue_prev": m_prev["omzet"],
            "mom_pct": mom_pct,
            "closing": m_now["count"],
            "closing_prev": m_prev["count"],
            "cash_saldo": cash,
            "piutang_total": piutang["p"] or 0,
            "piutang_count": piutang["n"] or 0,
            "revenue_target": rev_t,
            "closing_target": cls_t,
...
@app.get("/api/mgmt/company-targets")
...
@app.post("/api/mgmt/company-targets")
...
# ===========================================================================
# REKONSILIASI BANK (Payment Reconciliation)
# Alur: upload CSV mutasi bank -> parse -> insert Unmatched -> user pilih transaksi
# cocok (auto-suggest amount + date +-3 hari) -> Matched. Hanya finance/admin.
# ===========================================================================
_RECONCILE_ROLES = ("admin", "finance")


@app.post("/api/finance/reconcile/import")
async def reconcile_import(body: dict = Depends(json_body), user=Depends(authenticate_token)):
    ...
    require_role(user, *_RECONCILE_ROLES)
    ...
    for r in rows:
        ...
        try:
            ...
            # UNIQUE(row_hash) -> duplicate baris CSV yang persis sama (idempoten cross-batch)
            duplicates += 1
...
@app.get("/api/finance/reconcile/batches")
...
@app.get("/api/finance/reconcile/mutations")
...
@app.get("/api/finance/reconcile/mutations/{mid}/candidates")
async def reconcile_candidates(mid: int, user=Depends(authenticate_token)):
    ...
    # credit (uang masuk) -> match ke transactions type='income'
    # debit  (uang keluar) -> match ke transactions type='expense'
...
@app.post("/api/finance/reconcile/match")
...
@app.post("/api/finance/reconcile/unmatch")
...
@app.post("/api/finance/reconcile/ignore")
...
async def mgmt_home(user=Depends(authenticate_token)):
    ...
    # Omzet & closing bulan ini + bulan lalu
    def month_stat(ym):
        row = db.query_one(
            "SELECT COUNT(*) c, COALESCE(SUM(total_price),0) omzet FROM jamaah "
            "WHERE strftime('%Y-%m', COALESCE(order_date, created_at)) = ? "
    ...
    # Target company bulan ini
    company_tgt = db.query_one(
        "SELECT revenue_target, closing_target FROM company_targets WHERE month = ?",
        (ym_now,),
    ...
    # Sales attainment: target + actual bulan ini per sales
    sales_users = db.query_all(
        "SELECT id, name FROM users WHERE role = 'sales' ORDER BY name"
    )
    sales_perf = []
    for s in sales_users:
        actual = db.query_one(
        ...
        sales_perf.append({
            ...
            "target_closing": tc,
            "target_omzet": to,
            "closing_pct": round((c / tc) * 100) if tc else None,
            "omzet_pct": round((o / to) * 100) if to else None,
        })
    sales_perf.sort(key=lambda x: (x["actual_omzet"] or 0), reverse=True)
    ...
    # Approval inbox: yang butuh mgmt approve
    ...
    for r in refund_review:
        approvals.append({"kind": "refund", "id": r["id"], "label": f"Refund {r['jamaah_name'] or '-'}", "amount": r["amount"], "created": r["requested_at"]})
    for c in komisi_review:
        approvals.append({"kind": "komisi", "id": c["id"], "label": f"Komisi {c['agent_name'] or '-'} - {c['jamaah_name'] or '-'}", "amount": c["amount"], "created": c["requested_at"]})
    for i in incident_open:
        approvals.append({"kind": "incident", "id": i["id"], "label": f"[{i['severity']}] {i['package_name'] or '-'}: {i['incident_text'][:60]}", "amount": None, "created": i["created_at"]})

    # Attention
    attention = []
    n_ref = len(refund_review); n_kom = len(komisi_review); n_inc = len(incident_open)
    if n_inc:
        attention.append({"key": "incident_critical", "severity": "critical", "label": f"{n_inc} insiden Critical/High open", "goto": "incidents"})
    if n_ref:
        attention.append({"key": "refund_review", "severity": "high", "label": f"{n_ref} refund menunggu keputusan", "goto": "finance"})
    if n_kom:
        attention.append({"key": "komisi_review", "severity": "high", "label": f"{n_kom} klaim komisi menunggu keputusan", "goto": "finance"})
    # Sales under-perform check (kalau ada target dan actual < 50%)
    under = [s for s in sales_perf if s["closing_pct"] is not None and s["closing_pct"] < 50]
    if under and (now.day > 15):  # setelah tengah bulan
        attention.append({"key": "sales_under", "severity": "medium", "label": f"{len(under)} sales < 50% target closing bulan ini", "goto": "mgmt-home"})
...
# ===========================================================================
# RISK REGISTER (M-E) -- agregasi risk indicators cross-modul
# Untuk executive view: management + admin. Kategori Financial / Operational /
# Compliance. Severity CRITICAL/HIGH/MEDIUM/LOW mengikuti dampak bisnis.
# ===========================================================================
@app.get("/api/mgmt/risk-register")
async def mgmt_risk_register(user=Depends(authenticate_token)):
    require_role(user, "admin", "management")
    financial, operational, compliance = [], [], []

    # --- FINANCIAL ------------------------------------------------------------
    # Piutang H-14 (departure < 14 hari, belum lunas)
    r = db.query_one(
    ...
    # Piutang H-60 (31-60 hari) -- watch list, belum urgent
    r = db.query_one(
        "SELECT COUNT(*) c, COALESCE(SUM(COALESCE(j.total_price,0) - COALESCE(j.paid_amount,0)), 0) exposure "
        "FROM jamaah j LEFT JOIN packages p ON j.package_type = p.name "
        "WHERE j.payment_status IN ('DP','Unpaid') AND j.status NOT IN ('Cancelled','Lead - Follow Up') "
    ...
    # Vendor kontrak dengan sisa besar, deadline dekat (< 14 hari dari departure paket-nya)
    r = db.query_one(
    ...
    # --- OPERATIONAL ----------------------------------------------------------
    # Checklist paket H-7 < 70% completion
    ...
    if weak_checklist:
        operational.append({"severity": "CRITICAL", "key": "checklist_h7",
                            ...
                            "drilldown_page": "ops-home",
                            "detail": weak_checklist})

    # Vendor booking status != Confirmed, departure < 7 hari
    r = db.query_one(
    ...
    # Visa H-30 watchlist (15-30 hari)
    r = db.query_one(
        "SELECT COUNT(*) c FROM jamaah j LEFT JOIN packages p ON j.package_type = p.name "
        "WHERE j.status NOT IN ('Cancelled', 'Lead - Follow Up') "
        "  AND COALESCE(j.visa_status, '') NOT IN ('Approved', 'Issued') "
    ...
    # Debrief belum diisi untuk paket yang sudah pulang (return_date lewat)
    r = db.query_one(
        "SELECT COUNT(*) c FROM packages p "
        "WHERE p.return_date IS NOT NULL AND date(p.return_date) < date('now') "
        "  AND date(p.return_date) > date('now', '-60 days') "  # 60 hari terakhir saja
    ...
    # Kontrak vendor status Pending (menunggu approval)
    r = db.query_one(
